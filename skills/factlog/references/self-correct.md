...
- 타입 리터럴(날짜·금액·순위·수치)을 값으로 일치시키는 query는 저장된 **compound term**
  형식을 써야 매칭됩니다 — `date(2030,1)`, `ordinal(3)`, `number(2.5)`, 그리고 금액은
  단위를 따옴표로 둔 `amount(100,"억")`(query 문자열 안에서는 `\"`로 escape:
  `"amount(100,\"억\")"`). 프로즈(`"100억"`, `"2030.1"`)는 매칭되지 않으니, 값 추출이면
  object를 변수로 두십시오. 비교·임계 질문은 schema context에 해당 typed predicate가
  있을 때만 그 이름으로 질의하고, 없으면 원래 `review_required`를 둡니다.
