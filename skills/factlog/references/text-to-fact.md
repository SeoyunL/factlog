...
섹션으로 인정되는 것은 **코드펜스 밖의 ATX `## ` heading 뿐**입니다. 들여쓰기된 줄,
코드펜스 안의 줄, 그리고 밑줄식(Setext, `출처` 아래 `----`) heading 은 섹션이 아닙니다.
손으로 이 파일을 편집할 때 `## `로 시작하는 줄로 쓰십시오. Setext 는 **인식되지 않으며
이 동작은 개선되지 않았습니다**(#500) — 그 형식으로 쓴 섹션은 없는 것으로 취급됩니다.

코드펜스 안에 `## 출처 부족` 같은 **예시**를 적어 두는 것은 안전합니다. 도구는 그것을
섹션으로 세지 않고, bullet 도 거기로 보내지 않으며, 필요하면 진짜 섹션을 따로 만듭니다.
...
경고하고 그 파일을 건너뛰며, `validate.py` 는 `warning: unclosed_fence:` 로 같은 줄을
가리키고 **rc=1** 로 끝냅니다. 펜스를 닫고 다시 돌리면 그대로 이어집니다. 그 사이에도
해당 행은 `facts/candidates.csv` 에 `needs_review` 로 남아 있으므로 검토 큐 자체는
사라지지 않습니다.

> **동작 변경**: 이전에는 이런 파일에도 그냥 써서 검토 bullet 이 코드펜스 안에 묻힌 채
> rc=0 으로 끝났습니다. 이제는 쓰지 않고 rc=1 로 알립니다.
