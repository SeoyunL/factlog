...
3. **철회(`withdrawn_by`)는 두 모드 모두에서 자동 흡수되지 않습니다.** 사람에게 보고되고
   ...
   *기록*할 수는 있어도 *해제*할 수는 없습니다 — arXiv가 철회를 보고하지 않는다는 사실은
   ...
   못합니다. 해제는 사람이 프롬프트에서 노트를 보고 확인해야 합니다. 원장이 없는 논문은
   front matter에 `arxiv_version` 이 있으면 `arxiv-backfill-provenance` 로 원장을 만든 뒤
   종결할 수 있고, 없으면 백필이 거부하므로 어떤 명령으로도 종결하지 못합니다. 사람이 그
   논문의 `sources/*.md` front matter에 `arxiv_version: <N>` 을 손으로 추가해야 하며(`<N>` 은
   그 논문의 실제 arXiv 버전으로, `https://arxiv.org/abs/<id>` 에서 사람이 직접 읽습니다 —
   factlog가 대신 조회하지 않습니다), 그러면 백필이 원장을 만들 수 있고 철회를 종결할 수
   있습니다.
4. 버전을 비교할 수 없는 논문은 `unchanged` 가 아니라 **`no-version`** 이라는 자기 자신의
   상태로 보고됩니다. 비교된 것이 없으니 "변하지 않았다"고 말할 수 없습니다. 고치는 명령은
   원인마다 다르고, 고칠 명령이 아예 없는 경우도 있습니다 — [docs/arxiv.md](docs/arxiv.md) 의
   표를 보세요.
...
4. **삭제된 PMID는 조용히 버리지 않고 표시합니다**(네트워크 실패를 삭제로 오인하지
   ...
   돌려준 PMID를 둘 다 알립니다(레코드는 실제로 도착한 반환 PMID를 키로 기록하고, 요청→반환
   리다이렉트를 남겨 이후 `pubmed-refresh` 가 계보를 따라갈 수 있게 합니다).
