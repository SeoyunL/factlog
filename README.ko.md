...
**identity 관계.** 두 조건을 **모두** 만족할 때만 identity로 봅니다. (a)
`policy/attribute-relations.md` 에 리터럴 관계로 선언돼 있고, (b) 데이터가 **단사**입니다
— 모든 distinct 값이 정확히 한 주체에만 붙어 있습니다. 제목·DOI는 만족하지만, 발행연도는
리터럴로 선언돼 있어도 만족하지 않습니다(여러 논문이 2023년을 공유하므로). identity
관계에서 두 주체가 접힘값을 공유하면 중복 레코드일 가능성이 큽니다. 그 밖에서는 값이 여러
주체에 공유되는 게 정상이므로 충돌은 실제 질의 누수입니다. `--strict` 는 누수에만
실패합니다.

두 신호를 모두 요구하는 이유가 있습니다. 선언만 보면 `2023년`/`2023 년` 같은 진짜 분열이
"중복 레코드"로 강등되고, 단사성만 보면 사실이 두 개뿐인 KB에서는 무엇이든 identity가
됩니다.
