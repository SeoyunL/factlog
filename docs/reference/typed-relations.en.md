...
- `ordinal` — `3rd` / `3위` / `제3호` → int rank. **Engine-projectable**. A rank
  unit must follow the number — Korean `호`/`위`/`번`/`차`/`등`/`째` (with an
  optional leading `제`), or English `st`/`nd`/`rd`/`th`. Without a unit the
  value does not parse and stays untyped: `3`, `제3` and `rank 3` all fail.
