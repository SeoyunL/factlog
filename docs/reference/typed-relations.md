...
- `date` — `2030.1` / `2030-01-15` → 정렬 가능한 yyyymmdd. **엔진 투영 지원**
  (정렬/임계값/범위).
- `ordinal` — `3rd` / `3위` / `제3호` → 정수 순위. **엔진 투영 지원**. 숫자 뒤에
  순위 단위가 붙어야 파싱됩니다 — 한국어 `호`/`위`/`번`/`차`/`등`/`째`(앞의 `제` 는
  선택), 영어 `st`/`nd`/`rd`/`th`. 단위가 없으면 파싱되지 않아 값이 타입 없는
  상태로 남습니다: `3` 도 `제3` 도 `rank 3` 도 모두 미파싱입니다.
