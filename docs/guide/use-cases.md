...
실제 명령 흐름으로 풀면 다음과 같습니다. 모든 단계는 **Claude Code 세션 안에서**
실행합니다. 어느 경우든 골격은 같습니다 — `sync`/슬래시는 **후보(candidate)** 만
만들고, 사람이 `!factlog accept` 로 확정해야 **accepted** 가 되며, 그래야
`check`/`ask` 답변의 근거가 됩니다
...
> **`!` 표기.** Claude Code 프롬프트에서 줄 앞에 `!` 를 붙이면 그 줄은 셸 명령으로
> 실행됩니다. 검토·승인·폐기는 Claude 가 대신하지 않고 **사람이 직접** 이 방식으로
> 실행합니다 — 입구만 Claude Code 로 합쳐졌을 뿐, 사람의 게이트는 그대로입니다.

**보고서 검증하기**

- 보고서 파일을 KB의 `sources/` 에 둔다
- (Claude Code에서 실행) `/factlog sync` 로 후보 사실을 추출한다
- (Claude Code에서 `!` 로 실행) `!factlog review` 로 후보를 확인하고 `!factlog accept` 로 승인한다
...
- (Claude Code에서 `!` 로 실행) `!factlog review` 로 검토하고, 근거가 분명한 주장만 `!factlog accept` 로 승인한다
...
- (Claude Code에서 `!` 로 실행) `!factlog review` 로 검토하고 `!factlog accept` 로 확정한다
...
- (Claude Code에서 `!` 로 실행) `!factlog review` 로 새 후보를 확인하고 `!factlog accept` 로 승인한다
- (Claude Code에서 실행) `/factlog check` 로 누적된 accepted 사실의 일관성을 점검한다

**특정 사실의 출처 추적하기**

- (Claude Code에서 `!` 로 실행) `!factlog search` 로 확인하려는 사실을 찾는다
- (Claude Code에서 `!` 로 실행) `!factlog provenance` 로 그 사실이 어느 소스에서 왔는지 추적한다
...
- (Claude Code에서 `!` 로 실행) `!factlog review` 로 후보를 검토한다
- (Claude Code에서 `!` 로 실행) 잘못 추출된 후보는 `!factlog reject` 로 폐기하고, 표현만 다듬을 후보는 `!factlog amend` 로 값을 고친 뒤 `!factlog accept`(또는 `!factlog amend --accept`)로 승인한다
