...
2. **PubMed 자신의 신호를 그대로 표면화.** PubMed는 매핑하지 못한 구/필드를
   ...
   표면화 대상에서 제외합니다(태그 이름으로만 판정합니다). 신호에 붙는 조언의 분기는
   응답 본문의 문구가 아니라 **우리가 보낸 쿼리 문자열**을 근거로 합니다 — 응답을 문구
   매칭해 사용자의 입력을 추측하지 않습니다.
...
한 가지 실측 함정이 있습니다. PubMed는 **따옴표를 치지 않은 쿼리에도** `QuotedPhraseNotFound`
를 냅니다 — 경고 본문의 따옴표는 PubMed가 스스로 붙인 것이고, 같은 응답의 `QueryTranslation`
을 보면 ATM은 켜져 있습니다(예: 따옴표 없이 보낸 `qzxwvunonsenseterm` 이
`<QuotedPhraseNotFound>"qzxwvunonsenseterm"</QuotedPhraseNotFound>` 로 돌아옵니다). 그래서 이
태그가 곧 "사용자가 따옴표를 썼다"는 뜻은 아닙니다. factlog는 **우리가 보낸 쿼리가 PubMed가
지목한 바로 그 구를 따옴표로 감쌌을 때만** "따옴표를 빼면 느슨하게 매핑된다"고 안내합니다.
쿼리 전체에 따옴표가 있는지가 아니라 지목된 구 단위로 판정합니다 — `'"gene therapy" AND foo'`
처럼 한 구는 인용하고 다른 구는 안 한 쿼리에서 PubMed가 `foo` 를 지목했다면, 치지도 않은
...
#### 인용되는 원문은 한 줄로 정규화됩니다

세 블록 모두 **한 줄의 주장 + 들여쓴 이유 한 줄**이라는 모양 자체가 의미를 나릅니다. 그런데
인용되는 `MedlineDate` 는 PubMed가 준 자유텍스트이고 PMID도 응답에서 온 값이라, 거기 개행이
섞이면 블록이 쪼개지면서 **레코드 데이터가 `⚠` 로 시작하는 줄**로 올라앉습니다. 읽는 사람은
factlog가 한 말과 구별할 수 없습니다.

그래서 인용부의 탭·CR·LF는 각각 **공백 한 칸으로 치환**된 뒤 출력됩니다(`--porcelain` 행이
쓰는 것과 같은 규칙). 지우는 게 아니라 바꾸는 것이므로 감사에 필요한 원문은 그대로 남고, 블록은
항상 두 줄로 유지됩니다. 제어문자가 아닌 문자는 `⚠` 를 포함해 손대지 않습니다 — 줄을 위조하는
것은 줄바꿈이지 그 글자가 아닙니다.
