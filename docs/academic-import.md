...
DOI/PMID를 가진 원본을 **다른 연동이** 이미 썼으면 두 번째 파일을 쓰지 않고, 그 원본의
provenance 원장에 자기 레코드를 접습니다(`merged`). 같은 연동이 쓴 원본과 겹치는 것은
교차 소스 병합이 아니라 그냥 중복이므로 `skipped` 입니다 — DOI가 같은 arXiv 기탁본 둘이
서로의 원장에 접히지는 않습니다. `doi` / `arxiv_id` / `pmid` 가 (`openalex_doi` 같은
...
5. **철회·withdrawal 신호는 자동으로 흡수되지 않습니다.** 사람이 종결할 때까지 실행마다
   다시 올라옵니다 — 아래 [철회 신호](#철회는-연동마다-서로-다른-것을-뜻합니다) 참조.
...
기본값으로 되돌아갑니다) — 모든 요청에 그대로 실리는 값이라 오타가 조용히 익명 요청으로
떨어지면 안 되기 때문입니다. Zotero는 로컬 API라 `email` 설정 자체가 없습니다.
...
`pubmed-search --limit` 으로 조절하세요(`pubmed-import` 는 `--pmid` 를 반복해 최대
200건입니다).
...
## 철회는 연동마다 서로 다른 것을 뜻합니다

같은 "철회"라는 말이 서로 다른 절차를 가리킵니다. 세 **데이터베이스** 연동은 각각
**자기 이름이 붙은 front matter 키와 자기 종결 명령**으로 다루고, Zotero는 사람의 태그를
그대로 옮길 뿐 종결 명령이 없습니다.
...
| Zotero | 사람이 항목에 붙인 **태그** (`retract` 를 포함하는 태그면 참) | `retracted` — 철회 키 중 유일하게 접두어가 없습니다 | 없음 — 아래 참조 |

Zotero의 `retracted:` 키에 접두어가 없는 것은 상류 데이터베이스의 주장이 아니라 **사람이
자기 라이브러리에 남긴 판단**이기 때문입니다.

**종결 명령이 없는 대신, 이 값은 임포트 이후 갱신되지 않습니다.** factlog는 임포트한 뒤
Zotero를 다시 읽지 않으므로(`zotero-import` 가 유일한 Zotero 명령입니다), 라이브러리에서
태그를 붙이거나 지워도 KB에 전파되지 않습니다. 재임포트도 `zotero_key` 정체 일치에서
`skipped` 로 끝나 `.md` 를 다시 쓰지 않습니다. 값을 바꾸려면 `sources/*.md` 의 front
matter를 사람이 직접 고쳐야 합니다 — 아래 `arxiv_version` 수기 편집과 같은 성격입니다.
...
### `--yes` 는 기록할 수는 있어도 해제할 수는 없습니다 (#106)

상류가 철회를 더 이상 보고하지 않는다고 해서 그것이 곧 "진짜 되돌렸다"는 뜻은 아닙니다.
arXiv라면 철회 문장을 읽지 못한 것일 수 있고, PubMed라면 큐레이션이 늦은 것일 수 있으며,
코드는 이 둘을 구별하지 못합니다. OpenAlex의 `is_retracted` 는 구조화된 불리언이라 그런
읽기 실패는 없지만, 대신 OpenAlex는 오탐이 알려진 출처입니다 (#51) — 즉 사람이 노트를
읽고 판단할 이유는 세 곳 모두에 있습니다.

**기록은 소리를 내는 방향이고 해제는 침묵시키는 방향입니다.** 잘못 기록하면 성가신 데서
끝나지만 잘못 해제하면 철회된 논문을 인용하게 됩니다. 그래서 해제는 사람이 터미널에서
노트를 직접 보고 확인해야 하며, **세 종결 명령 모두** — `arxiv-acknowledge-withdrawal`,
`pubmed-acknowledge-retraction`, `openalex-acknowledge-retraction` — `--yes` 아래의 해제를
거부하고 아무것도 쓰지 않습니다. 해제하려면 `--yes` 없이 터미널에서 다시 실행하세요.
